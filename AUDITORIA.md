...
Fecha remediación: 2026-07-09
Alcance: backend (Django 5.1 + DRF), sitio público, panel de administración, API y seguridad.
Método: revisión de código real de `apps/*`, `config/settings/*`, `templates/*`, `static/*` y `seed.py`, seguida de remediación en ramas dedicadas.
...
| Hallazgo | Sev | Estado | Ref |
|----------|-----|:------:|:---:|
| [SEC-01](#sec-01) Precio de reserva manipulable | Crítico | ✅ | C |
| [SEC-02](#sec-02) Credenciales fijas reimpuestas en cada deploy | Crítico | ✅ | C |
| [SEC-03](#sec-03) `/init-soporte/` abierto a anónimos | Crítico | ✅ | C |
| [SEC-04](#sec-04) Cancelar/reseñar reservas ajenas por ID | Crítico | ✅ | C |
...
| [PUB-04](#pub-04) Enlaces relativos rompen navegación | Alto | ✅ | A2 |
...
1. **`SECRET_KEY` debe estar definida en Railway.** Tras [SEC-06](#sec-06) el arranque falla a propósito si falta.
2. **Rotar las contraseñas filtradas ya.** [SEC-02](#sec-02) evita que se reimpongan, pero las cuentas de producción siguen con las claves del repo hasta cambiarlas (panel o variables `SEED_CAMILO_PASSWORD`, `SEED_JUANDAVID_PASSWORD`, `SEED_SOPORTE_PASSWORD`, `SOPORTE_PASSWORD`).
3. **Definir `INIT_SOPORTE_TOKEN`** si se quiere seguir usando `/init-soporte/` (si no, queda deshabilitado).
4. Al arrancar de verdad los schedulers ([BIZ-01](#biz-01)/[BIZ-02](#biz-02)), **empezarán a salir los correos de recordatorio** en producción: verificar que el SMTP esté configurado.

---

## Resumen ejecutivo

Se detectaron **4 problemas de toma de control / manipulación explotables hoy mismo** que deben atenderse antes que nada (todos ✅ corregidos en la rama de críticos):
...
- **Estado:** ✅ Corregido (C). `price` eliminado de `BookingCreateSerializer`; el servidor fija `service.price`.
...
- **Estado:** ✅ Corregido (C). `seed.py` y `createsoporte` leen de variables de entorno y solo asignan clave al crear; sin plaintext en el repo. **Falta la acción operativa:** rotar las claves actuales (ver arriba).
...
- **Estado:** ✅ Corregido (C). Protegido con `INIT_SOPORTE_TOKEN`; deshabilitado si la variable no está definida.
...
- **Estado:** ✅ Corregido (C). Ambos endpoints exigen token firmado (`Signer`); se actualizaron el email de encuesta, la ruta `/rate/` y `rate.html`. Además se acotó el rating a 1–5 (ver [BIZ-10](#biz-10)).
...
- **Estado:** ✅ Corregido (C). Nuevo permiso `IsSuperAdminOrReadOnly`: lectura pública, escritura solo superadmin. La UI de Configuración aún muestra el campo a `admin`, pero el backend ahora lo rechaza (403).
...
- **Estado:** ✅ Corregido (C). En producción real (con `DATABASE_URL`) la `SECRET_KEY` es obligatoria; el arranque falla si falta. **Requiere** que la variable esté seteada en Railway.
...
- **Estado:** ✅ Corregido (A1). Los 4 endpoints admin de galería/reels ahora exigen `IsAdminOrAbove`.
...
- **Estado:** ✅ Corregido (A1). El panel y su contador se restringen a `operational_admin`/`superadmin` en frontend y backend (aprobar ventas no es competencia de `admin`).
...
- **Estado:** ✅ Corregido (A1). Deja pasar `Http404`/`PermissionDenied`; no expone `str(exception)` en producción (mensaje genérico) y loguea el detalle del lado servidor.
...
- **Estado:** ✅ Corregido (A1). Guard reescrito: arranca al servir la app (gunicorn), se salta comandos de gestión, `seed.py` y el padre del reloader.
...
- **Estado:** ✅ Corregido (A1). Guard alineado con el de bookings.
...
- **Estado:** ✅ Corregido (A1). Al borrar el cierre se elimina el egreso "Pago Diario: Franko" y se desmarcan sus comisiones (en transacción); el cierre solo paga comisiones de Frank no pagadas.
...
- **Estado:** ✅ Corregido (A1). El seed ya no sobreescribe el `price` de servicios existentes; sigue sincronizando el resto y auto-creando los que falten.
...
- **Estado:** ✅ Corregido (A1). `date = models.DateField(default=timezone.localdate)` + migración `0008_alter_expense_date`.
...
- **Estado:** ✅ Corregido (A2). `create_booking` y el reagendado serializan chequeo+guardado por barbero con `transaction.atomic()` + `select_for_update`. El envío de correos y el `201` se hacen tras el commit. Verificado: crear reserva → 201, mismo slot de nuevo → 409.
...
- **Estado:** ✅ Corregido (A1). `pay_barber_view` rechaza a Frank; el cierre filtra `is_paid=False` en las comisiones de Frank.
...
- **Estado:** ✅ Corregido (C). Se añadió un `clamp` 1–5 en `add_review_view` (junto al fix de [SEC-04](#sec-04)).
...
- **Estado:** ✅ Corregido (A2). Ya no marca `completed`: deja la cita `confirmed` (cobrable) y registra `completed_at`; guarda contra `None` y valida propiedad antes de desreferenciar `barber.user`. **Nota UX:** el dashboard del barbero podría requerir un ajuste menor para distinguir "atendida, pendiente de cobro" vía `completed_at`.
...
- **Problema:** cuando se arreglen los guards (BIZ-01/02), ambos workers ejecutan `ready()` → dos `BackgroundScheduler`. `send_upcoming_reminders` marca `reminder_sent` sin lock → recordatorios duplicados.
- **Corrección:** un solo proceso "leader" (env var / lock en BD) o mover los jobs a un cron externo de Railway.
- **Estado:** 🔶 Parcial (A1). Los recordatorios ahora **reclaman el envío de forma atómica** (`update(...)` condicional sobre `reminder_sent`), así que no se duplican entre workers. La consolidación ROI aún puede correr en ambos workers, pero es idempotente.
...
- **Estado:** ✅ Corregido (C). Resuelto en `styles.css`, `admin.js` y también en el duplicado muerto `admin_dashboard_new.html`.
...
- **Estado:** ✅ Corregido (C). Se añadió `esc()` y se aplica a nombre/comentario/servicio/barbero antes de `innerHTML`.
...
- **Estado:** ✅ Corregido (A2). También se protegió el `.catch` contra el loader ya removido.
...
- **Estado:** ✅ Corregido (A2). Enlaces relativos `.html` de `gallery.html` e `index.html` cambiados a rutas absolutas (`/booking/`, `/gallery/`).
...
- **Estado:** ✅ Corregido (A2). Se usa `Promise.allSettled` con `return`, tolerando fallos parciales.
...
- **Estado:** ✅ Corregido (A2). El botón se deshabilita ("Enviando...") y se re-habilita en error; `.catch` con feedback.
...
- **Estado:** ✅ Corregido (A2). Se quitó el `<script>` de `public/services.html` y se borró `static/js/services.js`.
...
- **Estado:** ✅ Corregido (C). `esc()` global en `base_admin.html`, aplicado en bookings, dashboard, clients, cashflow y audit_log. En `bookings.html` el `onclick` de COBRAR ya no interpola datos del cliente (lee de `window.__bookingsById`); en `clients.html` el teléfono va por `data-attribute`.
...
- **Estado:** ✅ Corregido (C) a nivel backend vía [SEC-05](#sec-05): la escritura de servicios es superadmin-only, así que un `admin` recibe 403 aunque la UI aún muestre el campo.
...
- **Estado:** ✅ Corregido (C) vía [SEC-01](#sec-01): el servidor siempre usa `service.price`.

<a id="adm-04"></a>
#### ADM-04 · ALTO · Dashboard usa la fecha del servidor (UTC)
- **Dónde:** `apps/analytics/views.py:140-144` (`date.today()` + `strptime` sin tz). En Railway (UTC), desde las ~7pm Colombia "hoy" es mañana: el KPI "Ingresos Hoy" va a 0 y el Kanban muestra el día equivocado. Mismo patrón en `barbers.html:616,816,914` (`new Date().toISOString()...`).
...
- **Estado:** ✅ Corregido (A1 backend + A2 JS). `dashboard_stats_view` usa `timezone.localdate()`; en `barbers.html` las 3 fechas "hoy" usan el offset local.
...
- **Estado:** ✅ Corregido (A2). `openCheckout()` resetea explícitamente todos los campos del cobro (evita el arrastre) y se valida el método de pago antes de enviar.

<a id="adm-06"></a>
#### ADM-06 · ALTO · Calendario muestra acciones que el backend rechaza en silencio
- **Dónde:** `templates/admin/calendar.html:56-58,161`. Muestra Confirmar/Completar/Cancelar y habilita drag&drop a **todos los roles**, pero el backend solo permite cambiar estado a superadmin/operational y fecha/hora a operational/superadmin. Para los demás el PATCH devolvía 200 ignorando los campos → el evento quedaba movido en pantalla pero no en la BD. `updateBookingStatus` no miraba errores 403/409.
...
- **Estado:** ✅ Corregido (A2). Botones y drag&drop gateados por rol (Confirmar/Cancelar: super+operational; Completar: super; mover fecha/hora: super+operational — alineado con el backend); se muestran errores 403/409 y se revierte el movimiento visual. El PATCH backend ahora devuelve 403 explícito.
...
- **Estado:** ✅ Corregido (A1) vía [SEC-09](#sec-09).
...
- **Tres fórmulas distintas de "Ingreso Neto" en Caja** (`apps/users/views.py:253`, `apps/cashflow/views.py:471-483` y `:150-158`): unificar en una sola función en `cashflow/services.py`.
- **Botón "Cerrar caja" queda muerto** (`cashflow.html:304,985`): habilitar/deshabilitar dentro de `cfUpdateKPIs()` y registrar el listener siempre.
- **Hora del cierre en UTC** (`apps/cashflow/views.py:294`): usar `timezone.localtime`.
- **Acciones sin manejo de error** (`quickStatus`, `toggleActive`, `deleteBlockedDate`/`deleteService`, subida de foto, `deleteBarber`).
- **Menú "Acciones" recortado por overflow** (`bookings.html:110-111`).
- **Montos sin separador de miles**: usar `Number(x).toLocaleString('es-CO')` (Reservas, Calendario, Clientes, dashboard).
...
- `</div>` extra en `bookings.html:132`; código muerto en `roi_dashboard.html:560`, `audit_log.html:165-167`, `bookings.html:582-595`.
...
Cada lote se validó con: `python manage.py check` (0 issues), `makemigrations --check` (sin migraciones pendientes), compilación de las plantillas editadas (sintaxis de tags Django), y pruebas funcionales con el test client:
...
*Documento vivo: refleja el estado de remediación al 2026-07-09. Cada hallazgo cita archivo:línea para verificación directa. Antes de desplegar, completar las acciones operativas (rotar credenciales, setear `SECRET_KEY` e `INIT_SOPORTE_TOKEN`) y confirmar cada flujo corregido con una prueba manual.*
